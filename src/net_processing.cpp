...
static void ProcessHeadersMessage(CNode& pfrom, CConnman& connman, ChainstateManager& chainman, CTxMemPool& mempool, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
    ...
    BlockValidationState state;
    if (!chainman.ProcessNewBlockHeaders(nPoSTemperature, pfrom->lastAcceptedHeader, headers, state, chainparams, &pindexLast)) {
        if (state.IsInvalid()) {
                                         ...
                                         const CChainParams& chainparams, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (!AlreadyHaveTx(GenTxid(/* is_wtxid=*/true, wtxid), m_mempool) &&
            AcceptToMemoryPool(m_mempool, state, ptx, false /* bypass_limits */, false /* test_accept */)) {
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!m_chainman.ProcessNewBlockHeaders(nPoSTemperature, ::ChainActive().Tip()->GetBlockHash(), {cmpctblock.header}, state, chainparams, &pindex)) {
            if (state.IsInvalid()) {
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        while (fContinue) {
            ...
            m_chainman.ProcessNewBlock(chainparams, pblock, forceProcessing, &fNewBlock, &pindexLastAccepted, &fPoSDuplicate);
            if (fNewBlock) {
                pfrom.nLastBlockTime = GetTime();
            } else {
                LOCK(cs_main);
