...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const uint32_t nSpendTime, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if (nMinimumSumAmount != std::numeric_limits<CAmount>::max()) {
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            AvailableCoins(*locked_chain, vAvailableCoins, true, &coin_control, txNew.nTime, 1, std::numeric_limits<CAmount>::max(), std::numeric_limits<CAmount>::max(), 0);
...
bool CWallet::CreateCoinStake(const CWallet* pwallet, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew)
{
    ...
    coin_selection_params.use_bnb=false;
    bool bnb_used;
    AvailableCoins(*locked_chain, vAvailableCoins, true, &temp, txNew.nTime, 1, std::numeric_limits<CAmount>::max(), std::numeric_limits<CAmount>::max(), 0);
