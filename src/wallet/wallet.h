...
public:
     ...
     */
    void AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe = true, const CCoinControl* coinControl = nullptr, const uint32_t nSpendTime = 0, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = std::numeric_limits<CAmount>::max(), const CAmount& nMinimumSumAmount = std::numeric_limits<CAmount>::max(), const uint64_t nMaximumCount = 0) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
